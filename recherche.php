		...
		<div id="recherche">
			<?php 
				if( (!empty($_POST['requete'])) || isset($_GET['request']) )
				{
					...
					$query = "SELECT * FROM `tp_film` LEFT JOIN tp_genre ON tp_film.id_genre = tp_genre.id_genre LEFT JOIN tp_distrib ON tp_film.id_distrib = tp_distrib.id_distrib WHERE `titre` LIKE '%$requete%' OR tp_genre.nom LIKE '%$requete%' OR tp_distrib.nom LIKE '%$requete%' ORDER BY `titre` ASC LIMIT 15 OFFSET ".$offset;
					$queryCount = "SELECT * FROM `tp_film` LEFT JOIN tp_genre ON tp_film.id_genre = tp_genre.id_genre LEFT JOIN tp_distrib ON tp_film.id_distrib = tp_distrib.id_distrib WHERE `titre` LIKE '%$requete%' OR tp_genre.nom LIKE '%$requete%' OR tp_distrib.nom LIKE '%$requete%' ORDER BY `titre` ASC";
